...
from .gnn_encoder import InfoNCELoss
from ..datasets import RerankerBatch


class Reranker(L.LightningModule):
    def __init__(
        self,
        cfg: GnnRankerConfig,
        inter_graph: Data,
    ...
    def forward(self, batch: RerankerBatch) -> torch.Tensor:
        scores = self.model(
            u_ids=batch.user_id,
    ...
    def training_step(self, batch: RerankerBatch) -> torch.Tensor:
        return self._step(batch, "train")

    def validation_step(self, batch: RerankerBatch):
        self._step(
            batch,
            "val",
    ...
    def test_step(self, batch: RerankerBatch):
        self._step(
            batch,
            "test",
    ...
    def predict_step(self, batch: RerankerBatch) -> torch.Tensor:
        return self(batch)
